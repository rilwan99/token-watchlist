...
- Symbol/name search returns 20 results by default; `limit` raises that and is honoured up to at least 200. It matches names as well as symbols, so a query that is a common English fragment ("sol" in "Solutions", "Solar") pulls in tokens unrelated to what was typed.
...
- One character is a real search, not a prefix guess: Jupiter ranks what it returns, so `s` returns SOL, Sonic, SPYx and `b` returns WBTC, cbBTC, xBTC. Verified against the live API on 2026-09-06. There is no junk-results reason to hold the panel back to two characters.
...
- Search results keep Jupiter's order and are never re-grouped or re-sorted; only the exact match moves. A trusted/untrusted split under an "Unverified · low liquidity" divider was tried and removed: verified against the live API on 2026-09-06, Jupiter returns the real token first for `USDC`, `BONK`, `JUP` and `TRUMP` and already arrives risk-stratified, so the split was a no-op on three of four and wrong on the fourth — it demoted verified `DJTx` (null liquidity) under a divider reading "Unverified", while `BONKGUY` at $15.9K sat under "low liquidity". Don't reintroduce a grouping rule coarser than the ranking it overrides.
- The one row a name search drops is one with null liquidity, in `searchUpstream`. It has no market: of the 14 that `sol` returns, none has a 24h change, nine have no price, and each holds 2-12 addresses — every column the app renders is blank, so they are empty results, not weak ones. Jupiter ranks them 4-17, so this overrides no ranking decision. Measured 2026-09-06 across twelve queries: 14 dropped on `sol`, 7 on `gold`, 1 each on `eth` and `trump`, zero on the other eight. This is not an asset-class rule — a tag filter on `rwa` was rejected because it deletes `NVDAx` ($1.86M liquidity, 71K holders) and `TSLAx`, which pass on their own numbers and rank first for their own symbols. Nothing in this file excludes an asset class; "Solana only" is a chain rule and tokenised equities are SPL mints.
- A mint query is exempt from that filter and from `limit`: `isMintQuery` treats a comma-separated list of base58 mints as the caller naming exact tokens. `fetchWatchlist` rehydrates through the same request shape, so filtering it would drop a watched token from the table while its mint stayed in storage, unable to return. A pasted mint therefore always resolves, including for a token with no market.
- `searchUpstream` sends `limit=50` on name searches; Jupiter's default of 20 is too few. `sol` spends 14 of the 20 on tokenised equities matching "Solutions"/"Solar" by name, which pushes JitoSOL, mSOL and PSOL to 18-20 and drops bSOL, dSOL, hSOL and BNSOL off the response entirely. Mint batches send no `limit` — verified 2026-09-06 that 25 mints return 25 without it, and the route already caps at 100.
- The exact symbol match pins to the top, but the pin means "this is what you typed", not "this is safe": the accent border and `Exact` tag render in accent ink only when the token is also verified, neutral otherwise. The pin is suppressed entirely when several unverified tokens share the symbol (`BONKGUY` returns three) — picking one is unsupportable. A verified exact match always wins the pin.
...
- Unverified rows read recessive: muted symbol, 40%-opacity icon, launchpad tag. No pill — one fixed-width slot holds `✓` when verified and `?` when not, so nothing after it shifts between rows. The slot always renders a glyph: with the divider gone this is the row's primary verification signal, and an empty slot signals by absence, which is too quiet to carry it.
