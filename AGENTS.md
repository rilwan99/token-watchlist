...
- Jupiter's icon URLs include IPFS gateway links (`ipfs.io`, `*.ipfs.dweb.link`) that answer with `Cross-Origin-Resource-Policy: same-origin`, so Chrome blocks them and logs `ERR_BLOCKED_BY_RESPONSE.NotSameOrigin` — 26 of them on a `bonk` search, none on a watchlist of SOL/USDC/BONK/JUP, since the count only tracks how many rendered tokens happen to carry an IPFS icon. No client-side change reaches that header, and `TokenIcon`'s initials fallback already renders those rows correctly, so the console noise is accepted. A proxy route was rejected — a URL-taking proxy needs an SSRF allowlist and a cache policy, real surface area for a cosmetic gain.
- `TokenIcon` remembers the URL that failed, not a boolean. Rows are keyed by mint so the instance outlives a refresh, and a `broken` flag meant a token whose icon 404'd once kept its initials until a reload even after Jupiter returned a working URL. Its initials take `labelClassName` per call site — 9/11/12px for the 22/28/32px circles — and read `—` when the stored symbol is empty, which storage permits.
...
- The desktop `<tr>` transitions `background-color` only, never the `transition-colors` shorthand. Its divider comes from the tbody's `divide-y divide-line`, which Tailwind scopes to `:not(:last-child)`, so the last row's border-color sits unset at the inherited `currentColor` — invisible only because its width is 0. A row that stops being last gains the 1px width instantly while the shorthand would animate that white down to `--color-line`: a near-white line flashing across the table on every add and every sort click. Keep the `divide-y` idiom and keep the transition narrow; don't fix this by colouring the last child or by moving to per-row `border-b`.
