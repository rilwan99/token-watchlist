...
**Ordering is Jupiter's, and only the exact match moves.** An earlier version split the list in two — verified and liquid on top, everything else under an "Unverified · low liquidity" divider. Checking it against the live API killed it. Jupiter ranks the real token first for `USDC`, `BONK`, `JUP` and `TRUMP`; the two tokens literally named `USDC` land at positions 7 and 8 on their own. The list already arrives sorted by risk, so the split was a no-op on three of those four queries — and on the fourth it was actively wrong, demoting verified `DJTx` (Trump Media, with null liquidity) below a divider that called it unverified. The label lied in the other direction too: `BONKGUY` at $15.9K of liquidity sat under "low liquidity". A two-bit rule layered over a ranking that weighs verification, liquidity, volume and organic score could only ever lose information.

**One row is dropped, and it isn't a ranking call.** Jupiter matches names as well as symbols, and defaults a search to 20 results. That combination breaks the most likely query this app will ever see. `sol` matches "**Sol**utions", "**Sol**ar" and "**Sol**stice", so 14 of those 20 slots go to tokenised equities — JitoSOL, mSOL and PSOL are pushed to ranks 18–20, and bSOL, dSOL, hSOL and BNSOL never arrive at all. Four of the seven visible rows were Ondo-tokenized US stocks.

Those 14 have something in common, and it isn't being stocks. None has a 24h change, the table's headline column. Nine have no price. They hold between two and twelve addresses each. Every column this app exists to render is blank for them, which makes them empty results rather than weak ones — so `searchUpstream` sends `limit=50` and drops rows with null liquidity. That leaves `sol` opening on SOL, JupSOL, SOLCAT, JitoSOL, mSOL, PSOL, bSOL.

The tempting version of this rule was a filter on Jupiter's `rwa` tag, since all 14 carry it. That was wrong for the same reason the divider was wrong: it's a category judgment layered over a ranking, and it deletes `NVDAx` — $1.86M of liquidity, 71,065 holders, first result for `nvda` — along with `TSLAx`, `SPYx` and `PAXG`. Tokenised equities are SPL mints that fill every column, so nothing about this project excludes them; "Solana only" is a rule about chains. Checking liquidity instead removes nothing from `usdc`, `bonk`, `jup`, `btc`, `pump`, `nvda`, `tsla` or `doge`.

Mint queries skip both the filter and the limit. Pasting an address is naming one exact token, and `fetchWatchlist` rehydrates the watchlist through that same request shape — filtering it would make a token that lost its liquidity vanish from the table while its mint sat in storage, unable to come back.

So the divider is gone and the row carries the whole story. Unverified rows read recessive: muted symbol, dimmed icon, launchpad as a tag, and a `?` in the same fixed-width slot that holds the verified `✓`. That slot always renders one glyph or the other, because the row's most important fact shouldn't be signalled by an absence — a missing checkmark is something you have to notice *isn't* there. The launchpad comes from Jupiter's `launchpad` field, not from sniffing the mint suffix, which lies in both directions: a `…bonk` suffix appears on verified letsbonk.fun launches, and plenty of pump.fun tokens carry no suffix at all.

**The pin answers "is this what you typed", not "is this safe".** The exact symbol match is pinned to the top, but the accent border and `Exact` tag only render in accent ink when that token is also verified; an unverified exact match keeps the position in neutral ink. And when several unverified tokens share the typed symbol the pin is suppressed entirely — `BONKGUY` returns three, and decorating one of them as *the* match is a claim nothing on the row can support.
