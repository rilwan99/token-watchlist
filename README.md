...
Built: load from storage, search with animated mobile results, add, remove with undo, manual refresh, the original desktop table, and the redesigned mobile accordion with animated details.
...
**Below 480px the watchlist uses the redesigned accordion; desktop keeps the original table.** The compact mobile row puts the token icon, symbol, verified check and truncated name beside a stacked price and 24h change, so a symbol collision can be resolved before expanding. It is the full tap target, with a down/up chevron, and only one mobile drawer is open at a time. Above that breakpoint, the existing table continues to show its original market columns and hover-revealed remove control.

**The mobile drawer answers whether the token still belongs on the list.** Market cap, liquidity and 24h volume lead in equal, tabular-number cells. Trust and context signals follow as wrapping chips: verification, holders, and launchpad where available. The mint and copy/remove icon buttons sit last. Remove has a red icon and border, never a dominant filled treatment; copying swaps its icon to a check. If opening a drawer pushes its bottom below the viewport, it scrolls into view after the expansion settles.
...
**Mobile price stays compact, and 24h change stops lying about zero.** A full-precision `$0.00005545` spends eighteen characters a narrow phone column cannot lend, so `formatPriceCompact` writes it as `$0.0₄5545` — subscript-zero notation, the subscript counting the zeros — and caps every output at nine characters. The desktop table keeps its full-precision formatter. Separately, a 24h change that rounds to `0.00%` renders neutral gray and drops its sign. A stablecoin sitting at +0.001% is not up, and painting it green says it is.
