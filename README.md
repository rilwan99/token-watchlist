...
**The slot is a fixed 416px and the panel fills it in every state.** Sizing the panel to its contents would jump ~370px a quarter-second after a keystroke, untied to anything you did — the precise failure the constraint exists to prevent. But a fixed slot alone just moves the problem: a one-row message at the top of it leaves ~350px of nothing between the panel and the watchlist. So loading fills the slot with skeleton rows built from the same grid as real results, pinned to the 53px a real row measures, so results land without moving a single column or row edge; no-match and failure centre their message instead. The emptiness that's left is enclosed by the panel's own border, which reads as a panel with little to say rather than an unexplained gap.
